...
import { Heart, Trash2, ShoppingCart, Clock, TrendingUp, Filter, Grid, List, Star } from 'lucide-react';
...
const Favorites = () => {
    ...
    const [favorites, setFavorites] = useState([
        ...
        {
            id: 2,
            name: 'โมเดล Batman Limited Edition',
            price: 4800,
            originalPrice: 6000,
            discount: 20,
        ...
        {
            id: 3,
            name: 'ของสะสม Transformers Vintage',
            price: 9500,
            originalPrice: 12000,
            discount: 21,
        ...
        {
            id: 4,
            name: 'ชุด Lego Star Wars Millennium Falcon',
            price: 12000,
            originalPrice: 15000,
            discount: 20,
        ...
        {
            id: 5,
            name: 'ฟิกเกอร์ Spider-Man Rare Edition',
            price: 5200,
            originalPrice: 6500,
            discount: 20,
        ...
        {
            id: 6,
            name: 'โมเดล Gundam RX-78-2 Ver. Ka',
            price: 3800,
            originalPrice: 4500,
            discount: 16,
                            ...
                            {sortedFavorites.map((item, index) => (
                                ...
                                >
                                    <div className={`relative ${viewMode === 'list' ? 'w-48 flex-shrink-0' : ''}`}>
                                        ...
                                        {/* Discount Badge */}
                                        {item.discount > 0 && (
                                            <div className="absolute top-3 left-3 bg-gradient-to-r from-red-500 to-pink-500 text-white px-2 md:px-3 py-1 rounded-full text-xs md:text-sm font-bold shadow-lg">
                                                -{item.discount}%
                                            </div>
                                        )}
                                        {/* Remove Button */}
                                    ...
                                    <div className={`p-3 md:p-5 flex flex-col ${viewMode === 'list' ? 'flex-1' : ''}`}>
                                        ...
                                        <div className="flex items-center space-x-1 mb-2">
                                            ...
                                            <span className="text-xs md:text-sm font-semibold text-gray-900">{item.rating}</span>
                                        ...
                                        <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-200">
                                            <div className="flex items-center space-x-1 text-gray-900">
                                                ...
                                                <span className="text-xs md:text-sm font-semibold">{item.bidCount} ผู้ประมูล</span>
                                            </div>
                                        ...
                                        <div className="flex items-center space-x-2 mt-auto">
                                            <button className="flex-1 bg-gray-900 hover:bg-black text-white py-2 md:py-2.5 px-3 md:px-4 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center space-x-1.5 md:space-x-2 text-xs md:text-sm font-semibold">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                <div className="text-2xl md:text-3xl font-bold">
                                    ฿{Math.min(...favorites.map(f => f.price)).toLocaleString()}
                                </div>
                                <div className="text-xs md:text-sm text-gray-300">ราคาต่ำสุด</div>
                            </div>
                            <div>
                                <div className="text-2xl md:text-3xl font-bold">
                                    ฿{Math.max(...favorites.map(f => f.price)).toLocaleString()}
                                </div>
                                <div className="text-xs md:text-sm text-gray-300">ราคาสูงสุด</div>
                            </div>
                            <div>
                                <div className="text-2xl md:text-3xl font-bold">
                                    ฿{Math.round(favorites.reduce((sum, f) => sum + f.price, 0) / favorites.length).toLocaleString()}
                                </div>
